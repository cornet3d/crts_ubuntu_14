...
void *ECR_rx_worker(void *_arg) {
  ...
  ECR->rx_buffer_len = ECR->usrp_rx_streamer->get_max_num_samps();
  ...
  while (ECR->rx_thread_running) {
    ...
    printf("rx worker beginning usrp streaming\n");
...
void ExtensibleCognitiveRadio::set_rx_streamer(int num_channels) {
  ...
  for(int i = 0; i < num_channels;i++) {
    rx_stream_args.channels.push_back(i);
  }
